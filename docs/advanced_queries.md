...
This file packages 15 rubric-friendly SQL queries against the Euroleague database. Run them in MySQL or phpMyAdmin after importing exports/euroleague_mysql_import.sql.

Each section includes:

- the SQL statement
- what the query proves or answers
- a screenshot reminder for the report

## Query 1

Season standings board with club and venue context.
...
Explanation: shows a complete standings table with season, team, country, arena, record, and point differential. This is a multi-table reporting query used for the public standings pages.

Screenshot: capture the result grid in phpMyAdmin after running the query.
...
Explanation: uses LEFT JOIN, GROUP BY, HAVING, COUNT, and AVG to show which countries host the strongest attendance environments.

Screenshot: capture the result grid in phpMyAdmin after running the query.

## Query 3

Players averaging at least 15 points with minimum game volume.
...
Explanation: identifies high-level scorers with enough sample size to be meaningful.

Screenshot: capture the result grid in phpMyAdmin after running the query.

## Query 4

Seasons and MVP winners using RIGHT JOIN.
...
Explanation: satisfies the RIGHT JOIN requirement while listing every season and its MVP winner when available.
...
Explanation: uses DISTINCT to reduce repeated country rows and show which nations reached the Final Four.
...
Explanation: shows venue usage and crowd strength while keeping arenas with partial game history in scope through LEFT JOIN.

Screenshot: capture the result grid in phpMyAdmin after running the query.

## Query 7

Head-to-head record by club pairing.
...
Explanation: compares rival teams across seasons and is an example of a 3+-table analytical query.

Screenshot: capture the result grid in phpMyAdmin after running the query.

## Query 8

Award totals by player and club.
...
Explanation: summarizes repeat winners and the club context attached to those awards.
...
Explanation: satisfies the subquery requirement and returns the top scorer for every season.

Screenshot: capture the result grid in phpMyAdmin after running the query.

## Query 10

Players above the season scoring average using a subquery.
...
Explanation: compares each player to the average scoring environment of that season.

Screenshot: capture the result grid in phpMyAdmin after running the query.

## Query 11

Clubs with repeated playoff qualification.
...
Explanation: identifies stable contenders across the archive.

Screenshot: capture the result grid in phpMyAdmin after running the query.

## Query 12

Coach records by club.
...
Explanation: aggregates coach performance by club and season count.

Screenshot: capture the result grid in phpMyAdmin after running the query.

## Query 13

Three-point specialists with minimum attempts.
...
Explanation: filters for shooters with both efficiency and sufficient volume.
...
Explanation: compares basketball strength at the country level using distinct club counts and winning records.
...
Explanation: gives you a fifteenth advanced query that still uses multiple tables, aggregation, LEFT JOIN, and HAVING while staying on your actual schema.
